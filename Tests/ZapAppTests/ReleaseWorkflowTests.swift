...
final class ReleaseWorkflowTests: XCTestCase {
    func testReleaseWorkflowBuildsAndUploadsSelfSignedDMG() throws {
        ...
        XCTAssertTrue(releaseLocal.contains("make VERSION=\"$VERSION\" BUILD_NUMBER=\"$BUILD_NUMBER\" BUILD_TAG=\"$RELEASE_TAG\" CODESIGN_IDENTITY=\"$CODESIGN_IDENTITY\" prepare-release-dmg"))
        ...
        XCTAssertTrue(secretRegistration.contains("CODESIGN_IDENTITY=\"zap\""))
        XCTAssertTrue(secretRegistration.contains("gh secret set"))
        XCTAssertTrue(secretRegistration.contains("openssl pkcs12 \"${legacy_args[@]}\" -export"))
        XCTAssertTrue(secretRegistration.contains("-passout \"file:${certificate_password_file}\""))
        XCTAssertTrue(secretRegistration.contains("gh secret set \"$CERTIFICATE_SECRET\" --repo \"$REPOSITORY\""))
        XCTAssertTrue(secretRegistration.contains("BEGIN (RSA |EC |)PRIVATE KEY"))
        ...
        XCTAssertTrue(workflow.contains("make CODESIGN_IDENTITY=\"$CODESIGN_IDENTITY\" VERSION=\"${{ steps.version.outputs.version }}\" BUILD_NUMBER=\"${{ steps.version.outputs.build_number }}\" BUILD_TAG=\"${{ steps.version.outputs.tag }}\" prepare-release-dmg"))
        XCTAssertFalse(workflow.contains("- name: Sign DMG"))
        ...
        assert("- name: Build, sign, and verify self-signed DMG", appearsBefore: "- name: Generate Sparkle appcast", in: workflow)
